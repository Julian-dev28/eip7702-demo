...
async function getSwapData() {
  try {
    ...
    const swapData2 = await client.dex.getSwapData({
      chainId: '8453', // Base chain ID
      fromTokenAddress: '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE',
      toTokenAddress: '0x0c41f1fc9022feb69af6dc666abfe73c9ffda7ce',
      amount: String(10 * 10 ** 18), // 0.0001 ETH (in wei)
      slippage: '0.5',     // 0.1%
      userWalletAddress: process.env.EVM_WALLET_ADDRESS
    });
    ...
    console.log(`Price impact: ${swapData2.data[0].routerResult.priceImpactPercentage}%`);
    console.log(`TX2 Data: ${swapData2.data[0].tx?.data}`);
